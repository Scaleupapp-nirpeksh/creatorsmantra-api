...
dealSchema.pre('save', function (next) {
  ...
  if (this.dealValue.tdsApplicable) {
    this.dealValue.tdsAmount = Math.round(this.dealValue.amount * 0.1)
  }

  // Calculate final amount
  this.dealValue.finalAmount =
    this.dealValue.amount + this.dealValue.gstAmount - this.dealValue.tdsAmount
...
brandProfileSchema.methods.updateCollaborationStats = async function () {
  ...
  if (deals.length > 0) {
    ...
    this.collaborationHistory.totalValue = deals.reduce(
      (sum, deal) => sum + deal.dealValue.amount,
      0
    )
    this.collaborationHistory.averageDealValue =
      this.collaborationHistory.totalValue / this.collaborationHistory.totalDeals
...
brandProfileSchema.virtual('brandScore').get(function () {
  ...
  if (history.lastCollaboration) {
    const daysSince = Math.floor(
      (Date.now() - history.lastCollaboration.getTime()) / (24 * 60 * 60 * 1000)
    )
