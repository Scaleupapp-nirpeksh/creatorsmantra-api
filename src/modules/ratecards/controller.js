...
const CACHE_KEYS = {
  ...
  AI_SUGGESTIONS: (metrics) =>
    `ai_suggestions:${crypto.createHash('md5').update(JSON.stringify(metrics)).digest('hex')}`,
...
class RateCardController {
  ...
  validationSchemas = {
      ...
      metrics: Joi.object({
        platforms: Joi.array()
          .items(
            Joi.object({
              name: Joi.string()
                .valid('instagram', 'youtube', 'linkedin', 'twitter', 'facebook')
                .required(),
              metrics: Joi.object({
                  ...
                  .try(
                    ...
                    Joi.string()
                      ...
                      .custom((value) => {
                        ...
                        if (num > 1000000000)
                          throw new Error('Followers cannot exceed 1,000,000,000')
                        return num
                      })
                  )
                  .required(),
    ...
    updateProfessionalDetails: Joi.object({
      paymentTerms: Joi.object({
        type: Joi.string()
          .valid('100_advance', '50_50', '30_70', 'on_delivery', 'net_15', 'net_30', 'custom')
          .optional(),
      ...
      usageRights: Joi.object({
        duration: Joi.string()
          .valid('1_month', '3_months', '6_months', '1_year', 'perpetual', 'custom')
          .optional(),
        platforms: Joi.array()
          .items(
            Joi.string().valid(
  ...
  checkOwnership = async (rateCardId, userId, allowManager = false) => {
    ...
    if (!isOwner && !isManager) {
      ...
      if (!creatorProfile) {
        throw new AppError(ERROR_CODES.NO_PERMISSION.message, 403, ERROR_CODES.NO_PERMISSION.code)
      }
    }
...
${metrics.platforms
  .map(
    (p) =>
      `- ${p.name}: ${p.metrics.followers.toLocaleString()} followers, ${
...
Market Context:
- Niche: ${metrics.niche} (${
      ['tech', 'finance', 'fashion', 'beauty'].includes(metrics.niche)
        ? 'Premium niche +20-30%'
        : 'Standard niche'
    })
- Location: ${metrics.location.city} (${metrics.location.cityTier}) ${
      metrics.location.cityTier === 'metro' ? '- Metro premium +30-50%' : ''
    }
...
${Object.entries(expectedRanges)
  .map(
    ([platform, rates]) =>
      `${platform.toUpperCase()}: ${Object.entries(rates)
        .map(
          ([type, range]) => `${type} ₹${range[0].toLocaleString()}-${range[1].toLocaleString()}`
        )
        .join(', ')}`
  )
  .join('\n')}
...
Return JSON with this exact structure:
{
    ...
    metrics.platforms.forEach((platform) => {
      ...
      Object.entries(baseRates[platform.name] || {}).forEach(([type, baseRate]) => {
        ...
        pricing.platforms[platform.name][type] = {
          ...
          reasoning: `Calculated based on ${followers.toLocaleString()} followers, ${engagement}% engagement, and market standards for ${
            metrics.location.cityTier
          } city`,
        }
      })
    })
    ...
    pricing.marketInsights = {
      ...
      averageRate: Math.round(
        Object.values(pricing.platforms)
          ...
          Object.values(pricing.platforms).flatMap((p) => Object.values(p)).length
      ),
    }
  ...
  buildPDFContent = async (doc, rateCard) => {
    ...
    // Creator Info
    doc.fillColor('#1F2937').fontSize(20).text(rateCard.creatorId.fullName, { align: 'center' })
    ...
    if (rateCard.description) {
      doc.fontSize(10).text(rateCard.description, { align: 'center' })
    }
    ...
    // Metrics Section
    doc.fillColor('#1F2937').fontSize(16).font('Helvetica-Bold').text('CREATOR METRICS')
    ...
    rateCard.metrics.platforms.forEach((platform) => {
      doc
        ...
        .fillColor('#1F2937')
        .text(
          `: ${platform.metrics.followers.toLocaleString()} followers | ${
    ...
    // Pricing Section
    doc.fillColor('#1F2937').fontSize(16).font('Helvetica-Bold').text('DELIVERABLE PRICING')
    ...
    rateCard.pricing.deliverables.forEach((platform) => {
      ...
      platform.rates.forEach((rate) => {
        doc
          ...
          .text(`₹${rate.pricing.userRate.toLocaleString('en-IN')}`, 400, yPosition, {
            width: 95,
            align: 'right',
          })
    ...
    if (rateCard.packages && rateCard.packages.length > 0) {
      ...
      doc.fillColor('#1F2937').fontSize(16).font('Helvetica-Bold').text('PACKAGE DEALS')
      ...
      rateCard.packages.forEach((pkg) => {
        ...
        doc.fillColor(primaryColor).fontSize(13).font('Helvetica-Bold').text(pkg.name)

        if (pkg.description) {
          doc.fillColor(secondaryColor).fontSize(10).font('Helvetica').text(pkg.description)
        }
        ...
        doc
          .fontSize(12)
          .text(`Package Price: ₹${pkg.pricing.packagePrice.toLocaleString('en-IN')}`, {
            continued: true,
          })

        if (pkg.pricing.savings.percentage > 0) {
          doc.fillColor(accentColor).text(` (Save ${pkg.pricing.savings.percentage}%)`)
        } else {
          doc.text('')
        }
    ...
    doc.moveDown(0.5).fontSize(10).font('Helvetica').fillColor(secondaryColor)

    const terms = rateCard.professionalDetails || {}

    doc
      .text(`• Payment Terms: ${this.formatPaymentTerms(terms.paymentTerms?.type)}`)
    ...
    if (terms.additionalNotes) {
      doc.moveDown().text('Additional Notes:', { underline: true }).text(terms.additionalNotes)
    }
    ...
    doc
      ...
      .text(`Document ID: ${rateCard.sharing.publicId || rateCard._id}`, 50, footerY + 12, {
        align: 'center',
        width: 495,
      })
      .text(
  ...
  createRateCard = asyncHandler(async (req, res) => {
    try {
      ...
      // TEMP
      //  REASON: Tracnscations were failing
  ...
  getRateCards = asyncHandler(async (req, res) => {
    try {
      ...
      if (!cachedData || cachedData?.length === 0) {
        // Build query
        const query = {
          $or: [{ creatorId: req.user.id }, { managedBy: req.user.id }],
          isDeleted: false,
        }
  ...
  getRateCard = asyncHandler(async (req, res) => {
    try {
      ...
      res.json(successResponse('Rate card fetched successfully', { rateCard }))
    } catch (error) {
  ...
  updateMetrics = asyncHandler(async (req, res) => {
    try {
      ...
      // Sanitize and validate
      const validatedData = this.sanitizeAndValidate(this.validationSchemas.updateMetrics, req.body)
  ...
  updatePricing = asyncHandler(async (req, res) => {
    try {
      ...
      res.json(successResponse('Pricing updated successfully', { rateCard }))
    } catch (error) {
  ...
  createPackage = asyncHandler(async (req, res) => {
    try {
      ...
      // Sanitize and validate
      const validatedData = this.sanitizeAndValidate(this.validationSchemas.createPackage, req.body)
  ...
  updatePackage = asyncHandler(async (req, res) => {
    try {
      ...
      // Sanitize and validate
      const validatedData = this.sanitizeAndValidate(this.validationSchemas.updatePackage, req.body)
      ...
      await this.withTransaction(async (session) => {
        ...
        if (validatedData.packagePrice !== undefined) {
          ...
          pkg.pricing.savings.percentage =
            pkg.pricing.individualTotal > 0
              ? Math.round(
                  ((pkg.pricing.individualTotal - pkg.pricing.packagePrice) /
                    pkg.pricing.individualTotal) *
                    100
                )
              : 0
        }
  ...
  updateProfessionalDetails = asyncHandler(async (req, res) => {
    try {
      ...
      res.json(successResponse('Professional details updated successfully', { rateCard }))
    } catch (error) {
  ...
  publishRateCard = asyncHandler(async (req, res) => {
    try {
      ...
      try {
        ...
        throw new AppError(ERROR_CODES.QR_CODE_ERROR.message, 500, ERROR_CODES.QR_CODE_ERROR.code)
      }
  ...
  getPublicRateCard = asyncHandler(async (req, res) => {
    try {
      ...
      res.json(successResponse('Public rate card fetched successfully', publicData))
    } catch (error) {
  ...
  generatePDF = asyncHandler(async (req, res) => {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="rate-card-${rateCard.sharing.publicId || rateCard._id}.pdf"`
      )
  ...
  restoreFromHistory = asyncHandler(async (req, res) => {
    try {
      ...
      res.json(successResponse('Rate card restored successfully', { rateCard }))
    } catch (error) {
  ...
  deleteRateCard = asyncHandler(async (req, res) => {
    try {
      ...
      res.json(successResponse('Rate card deleted successfully'))
    } catch (error) {
  ...
  getAnalytics = asyncHandler(async (req, res) => {
    try {
      ...
      const analytics = {
        ...
        performance: {
          ...
          deliverableCount: rateCard.pricing.deliverables.reduce(
            (count, platform) => count + platform.rates.length,
            0
          ),
        },
        aiInsights: {
      ...
      res.json(successResponse('Analytics fetched successfully', { analytics }))
    } catch (error) {
    ...
    deliverables.forEach((platform) => {
      platform.rates.forEach((rate) => {
        ...
        if (rate.pricing.aiSuggested && rate.pricing.userRate) {
          const variance =
            Math.abs(rate.pricing.userRate - rate.pricing.aiSuggested) / rate.pricing.aiSuggested
          if (variance <= 0.1) {
            acceptedRates++
          }
        }
