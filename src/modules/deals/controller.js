...
class DealController {
  ...
  deleteDeal = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(successResponse('Deal deleted successfully', {
      dealId: deal._id,
  ...
  archiveDeal = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(successResponse(
      `Deal ${deal.isArchived ? 'archived' : 'unarchived'} successfully`,
      {
        dealId: deal._id,
  ...
  getDealsNeedingAttention = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(successResponse('Deals needing attention retrieved', {
      deals: enrichedDeals,
      count: enrichedDeals.length,
      categories: {
        overdueResponse: enrichedDeals.filter(d => d.stage === 'pitched' && d.isOverdue).length,
        overdueContent: enrichedDeals.filter(d => ['live', 'in_talks'].includes(d.stage) && d.isOverdue).length,
        overduePayment: enrichedDeals.filter(d => d.stage === 'completed' && d.isOverdue).length
      }
    }));
  });
  ...
  getDealCommunications = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(successResponse('Deal communications retrieved', {
      dealId: deal._id,
      communications,
  ...
  updateCommunication = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(successResponse('Communication updated successfully', {
      dealId: deal._id,
  ...
  updateDeliverable = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(successResponse('Deliverable updated successfully', {
      dealId: deal._id,
  ...
  addDeliverable = asyncHandler(async (req, res) => {
    ...
    res.status(201).json(successResponse('Deliverable added successfully', {
      dealId: deal._id,
  ...
  updateBrandProfile = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(successResponse('Brand profile updated successfully', {
      brandId: brand._id,
      name: brand.name,
  ...
  updateDealTemplate = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(successResponse('Deal template updated successfully', {
  ...
  deleteDealTemplate = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(successResponse('Deal template deleted successfully', {
      templateId: template._id
    }));
  });
  ...
  getDealSummary = asyncHandler(async (req, res) => {
    ...
    summary.conversionRate = summary.totalDeals > 0 
      ? Math.round((summary.paidDeals / summary.totalDeals) * 100) 
      : 0;

    res.status(200).json(successResponse('Deal summary retrieved', {
      summary,
      stageBreakdown,
      period
    }));
  ...
  getDealMetadata = asyncHandler(async (req, res) => {
    const metadata = {
      stages: [
        ...
        { value: 'in_talks', label: 'In Talks', description: 'Brand showed interest, discussing details' },
        { value: 'negotiating', label: 'Negotiating', description: 'Formal negotiations in progress' },
        { value: 'live', label: 'Live', description: 'Campaign is active, creating content' },
        { value: 'completed', label: 'Completed', description: 'Content delivered, awaiting payment' },
  ...
  bulkUpdateDeals = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(successResponse('Deals updated successfully', {
