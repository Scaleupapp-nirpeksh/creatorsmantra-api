...
const createDealSchema = Joi.object({
  ...
  campaignRequirements: Joi.object({
    ...
    contentGuidelines: Joi.object({
      ...
      tone: Joi.string().valid('professional', 'casual', 'humorous', 'educational', 'inspirational'),
    ...
    usageRights: Joi.object({
      ...
      geography: Joi.string()
        .valid('india', 'global', 'specific_regions')
        .default('india'),
  ...
  source: Joi.string()
    .valid('direct_outreach', 'brand_inquiry', 'referral', 'social_media', 'email_campaign', 'networking', 'repeat_client', 'other')
...
const updateDealSchema = Joi.object({
  ...
  dealValue: Joi.object({
    ...
    paymentTerms: Joi.string().valid('full_advance', '50_50', '30_70', 'on_delivery', 'net_30', 'net_15', 'custom'),
...
const updateDealStageSchema = Joi.object({
  stage: Joi.string()
    .valid('pitched', 'in_talks', 'negotiating', 'live', 'completed', 'paid', 'cancelled', 'rejected')
    .required()
    .messages({
...
const updateBrandProfileSchema = Joi.object({
  ...
  preferences: Joi.object({
    ...
    campaignTypes: Joi.array().items(
      Joi.string().valid('brand_awareness', 'product_launch', 'seasonal', 'influencer_takeover', 'ugc', 'review', 'tutorial', 'lifestyle')
    ),
    contentStyle: Joi.array().items(
      Joi.string().valid('professional', 'casual', 'humorous', 'educational', 'inspirational', 'trendy')
    ),
  ...
  status: Joi.string()
    .valid('active', 'potential', 'blacklisted', 'inactive')
    .messages({
...
const createDealTemplateSchema = Joi.object({
  ...
  category: Joi.string()
    .valid('instagram_post', 'instagram_reel', 'youtube_video', 'brand_collaboration', 'product_review', 'custom')
    .default('custom')
    .messages({
...
const updateDealTemplateSchema = Joi.object({
  ...
  description: Joi.string().trim().max(500),
  category: Joi.string().valid('instagram_post', 'instagram_reel', 'youtube_video', 'brand_collaboration', 'product_review', 'custom'),
  platform: Joi.string().valid('instagram', 'youtube', 'linkedin', 'twitter', 'facebook', 'snapchat', 'multiple'),
...
const getDealsQuerySchema = Joi.object({
  stage: Joi.alternatives().try(
    Joi.string().valid('pitched', 'in_talks', 'negotiating', 'live', 'completed', 'paid', 'cancelled', 'rejected'),
    Joi.array().items(Joi.string().valid('pitched', 'in_talks', 'negotiating', 'live', 'completed', 'paid', 'cancelled', 'rejected'))
  ),

  status: Joi.string().valid('active', 'paused', 'completed', 'cancelled'),
  platform: Joi.string().valid('instagram', 'youtube', 'linkedin', 'twitter', 'facebook', 'snapchat', 'multiple'),
  priority: Joi.string().valid('low', 'medium', 'high', 'urgent'),
  source: Joi.string().valid('direct_outreach', 'brand_inquiry', 'referral', 'social_media', 'email_campaign', 'networking', 'repeat_client', 'other'),
  ...
  sortBy: Joi.string().valid('createdAt', 'updatedAt', 'dealValue.amount', 'priority', 'stage', 'brand.name').default('createdAt'),
...
const bulkUpdateDealsSchema = Joi.object({
  ...
  updateData: Joi.object({
    stage: Joi.string().valid('pitched', 'in_talks', 'negotiating', 'live', 'completed', 'paid', 'cancelled', 'rejected'),
    priority: Joi.string().valid('low', 'medium', 'high', 'urgent'),
    status: Joi.string().valid('active', 'paused', 'completed', 'cancelled'),
    tags: Joi.array().items(Joi.string().trim().min(1).max(50)).max(10).unique()
  })
...
const stageParamSchema = Joi.object({
  stage: Joi.string()
    .valid('pitched', 'in_talks', 'negotiating', 'live', 'completed', 'paid', 'cancelled', 'rejected', 'all')
    .required()
    .messages({
